...
export default function LiquidGrass({ children, inline = false, width = 210, height = 150, className = '' }: { children?: React.ReactNode; inline?: boolean; width?: number; height?: number; className?: string }) {
  ...
  const content = (
    <motion.div
      ...
      style={{ width, height }}
      className={`${inline ? 'relative' : 'fixed top-16 right-1/2 z-90 select-none'} ${className}`}
      whileTap={{ scale: 1.02 }}>
      ...
      {/* children container */}
      <div className='relative z-10 h-full w-full flex items-center justify-center p-3'>{children}</div>
    </motion.div>
  )
  ...
  if (typeof document === 'undefined' || !bodyRef.current) return null
